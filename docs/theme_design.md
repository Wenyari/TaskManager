...
>
> 修订记录
> - v1.1 (2026-05-25)：移除"书架横纹"背景纹理，改为纯色背景。横纹与卡片网格在视觉上互相干扰、信息密度过高，纯色更利于阅读。
...
- **背景纯色**：主背景使用纯羊皮纸色 / 深胡桃木色，不叠加纹理；卡片左侧"书脊"色带承担装饰
...
- 不使用纯黑 `#000` / 纯白 `#FFF`
- 不引入复杂图片纹理
- 不在主背景叠加横纹/网格等装饰图层（v1.0 曾尝试用 `repeating-linear-gradient` 做书架横纹，但与卡片网格视觉打架，v1.1 移除）
...
[TaskView.vue](apps/frontend/src/views/TaskView.vue) `NPageHeader` 标题显示用户名，空状态文案改成"点击「新建任务」添置"。
...
### D-T05 不引入图片纹理资源
**立场：纯色背景 + CSS 装饰边。** 图书馆质感容易被联想成"做一张木纹背景图加上去"，但图片资源会增加首屏体积、不响应主题、缩放走样。v1.0 曾用 `repeating-linear-gradient` 在 body 上画书架横纹，但与卡片网格视觉打架，信息密度过高、阅读疲劳，v1.1 移除。装饰感全部由卡片左侧的"书脊色带"承担，背景纯色更清爽，主题切换时也不需要额外的颜色变量参与。
